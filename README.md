...
Reply with the code plus text to paste the text into the agent and press Enter
without changing the focused tmux pane:
...
For `PermissionRequest` events, the daemon can bridge approval back to Codex
when the hook can identify a tmux route and register a pending approval. In
that case, the XMPP prompt includes:

```text
allow: /choose <route-code> 1 (direct) or /choose 1 (room)
deny: /choose <route-code> 2 (direct) or /choose 2 (room)
```

In direct chat, reply with `/choose <route-code> 1` or `/choose <route-code> 2`.
In a route-bound room, reply with `/choose 1` or `/choose 2` in the room. The
Codex hook waits for that reply and returns Codex's official allow/deny hook
JSON. If the daemon is not running, no tmux route is available, or the approval
times out, Codex falls back to its normal local approval UI.
...
focuses the tmux pane for the selected route. `/choose <route-code> <number>`
selects a pending prompt option. `/new [route-code]` starts a fresh Codex
session in a new tmux window, using the same working directory and tmux session
as the selected route. When `route-code` is omitted, the most recently active
route is used. The daemon reply includes the new route code for the fresh
window.
