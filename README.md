...
`PermissionRequest` events are delivered as normal XMPP notifications. Replying
from XMPP sends text to the routed tmux pane, so Codex's normal local approval
...
the server does not acknowledge the destroy request, `xmpp-cli` still marks the
room closed locally and says so in the close acknowledgement. If the bound route
or tmux pane is gone, the room remains open and replies that the route is no
longer active. Closing a room writes a local transcript under the agent room
log directory and includes that path in the close acknowledgement.
