...
(deftest codex-permission-decision-json-uses-official-shape
  (let* ((allow (xmpp-cli/agent-codex:codex-permission-decision-json
                 :allow))
         (deny (xmpp-cli/agent-codex:codex-permission-decision-json
                :deny
                :message "No."))
         (allow-json (xmpp-cli/json:parse-json allow))
         (deny-json (xmpp-cli/json:parse-json deny))
         (allow-output (xmpp-cli/json:json-value allow-json
                                                 "hookSpecificOutput"))
         (deny-output (xmpp-cli/json:json-value deny-json
                                                "hookSpecificOutput"))
         (allow-decision (xmpp-cli/json:json-value allow-output "decision"))
         (deny-decision (xmpp-cli/json:json-value deny-output "decision")))
    (check-equal "PermissionRequest"
                 (xmpp-cli/json:json-value allow-output "hookEventName"))
    (check-equal "allow"
                 (xmpp-cli/json:json-value allow-decision "behavior"))
    (check-equal "deny"
                 (xmpp-cli/json:json-value deny-decision "behavior"))
    (check-equal "No."
                 (xmpp-cli/json:json-value deny-decision "message"))))

(deftest codex-hook-installer-permission-timeout-covers-approval-wait
  (let* ((script (xmpp-cli/util:read-file-as-string
    ...
    (check timeout
           "PermissionRequest hook timeout should exceed the daemon approval wait")))

(deftest daemon-permission-request-direct-choose-allows-hook
  (with-isolated-data
    (let* ((now (xmpp-cli/util:now-iso8601))
           ...
           (request-error nil)
           (thread nil)
           (data-directory xmpp-cli/util::*data-directory*))
      (xmpp-cli/agent-routes:save-routes (list route))
      (setf *fake-events* nil)
      (unwind-protect
           (progn
             (setf thread
                   (bt:make-thread
                    (lambda ()
                      (let ((xmpp-cli/util::*data-directory* data-directory))
                        (handler-case
                            (setf response
                                  (xmpp-cli/agent-daemon::handle-control-request
                                   state
                                   (list :token "token"
                                         ...
                                         :route-id "route-1"
                                         :fallback-to "friend@example.org"
                                         :bodies '("permission prompt")
                                         :timeout-seconds 2
                                         :expected-profile-digest digest)))
                          (error (condition)
                            (setf request-error condition)))))
                    :name "xmpp-cli test permission direct"))
             (check
              (wait-for-test-condition
               (lambda ()
                 (some (lambda (event)
                         (and (eq (first event) :send-connected-text)
                              (string= (second event) "friend@example.org")
                              (string= (third event) "permission prompt")))
                       *fake-events*)))
              "permission request should send its prompt before waiting")
             (xmpp-cli/agent-daemon::handle-incoming-message
              state
              (list :kind :message
                    :from "friend@example.org/phone"
                    :body "/choose abcd 1"))
             (check
              (wait-for-test-condition
               (lambda ()
                 (and thread (not (bt:thread-alive-p thread)))))
              "permission request should finish after a numeric reply")
             (check (null request-error)
                    "permission request should not signal: ~a"
                    request-error)
             (check (getf response :ok)
                    "permission request should return ok")
             (check-equal :allow (getf response :decision))
             (check
              (some (lambda (event)
                      (and (eq (first event) :send-connected-text)
                           (search "selected option 1" (third event))))
                    *fake-events*)
              "direct approval reply should be acknowledged"))
        (when (and thread (bt:thread-alive-p thread))
          (ignore-errors
            (bt:destroy-thread thread)))))))

(deftest daemon-permission-request-room-choose-denies-hook
  (with-isolated-data
    (let* ((now (xmpp-cli/util:now-iso8601))
           (profile (list :jid "bot@example.org"))
           (digest (xmpp-cli/agent-ipc:profile-digest profile))
           ...
           (progn
                    ...
                    (lambda ()
                      (let ((xmpp-cli/util::*data-directory* data-directory))
                        (handler-case
                          ...
                          (error (condition)
                            (setf request-error condition)))))
                    :name "xmpp-cli test permission room"))
             (check
              (wait-for-test-condition
               (lambda ()
                 (some (lambda (event)
                         (and (eq (first event) :send-room-message)
                              ...
                              (string= (third event) "permission prompt")))
                       *fake-events*)))
              "permission request should route its prompt to the bound room")
             (xmpp-cli/agent-daemon::handle-room-message
              state
              (list :kind :groupchat
                    :from "room@groups.example.org/friend"
             ...
             (check
              (wait-for-test-condition
               (lambda ()
                 (and thread (not (bt:thread-alive-p thread)))))
              "permission request should finish after a room numeric reply")
             (check (null request-error)
                    "permission request should not signal: ~a"
                    request-error)
             (check (getf response :ok)
                    "permission request should return ok")
             (check-equal :deny (getf response :decision))
             (check (search "Denied from XMPP reply"
                            (getf response :message))
                    "deny decision should carry a Codex denial message")
             (check
              (some (lambda (event)
                      (and (eq (first event) :send-room-message)
                           (search "selected option 2" (third event))))
                    *fake-events*)
              "room denial reply should be acknowledged"))
        (when (and thread (bt:thread-alive-p thread))
          (ignore-errors
            (bt:destroy-thread thread)))))))

(deftest daemon-permission-request-room-prompt-direct-choose-allows-hook
  (with-isolated-data
    (let* ((now (xmpp-cli/util:now-iso8601))
           (profile (list :jid "bot@example.org"))
           (digest (xmpp-cli/agent-ipc:profile-digest profile))
           ...
           (progn
                    ...
                    (lambda ()
                      (let ((xmpp-cli/util::*data-directory* data-directory))
                        (handler-case
                          ...
                          (error (condition)
                            (setf request-error condition)))))
                    :name "xmpp-cli test permission room direct"))
             (check
              (wait-for-test-condition
               (lambda ()
                 (some (lambda (event)
             ...
             (check
              (wait-for-test-condition
               (lambda ()
                 (and thread (not (bt:thread-alive-p thread)))))
              "permission request should finish after a direct choose command")
             (check (null request-error)
                    "permission request should not signal: ~a"
                    request-error)
             (check (getf response :ok)
             ...
             (check
              (some (lambda (event)
                      (and (eq (first event) :send-connected-text)
                           (string= (second event) "friend@example.org/phone")
                           (search "selected option 1" (third event))))
                    *fake-events*)
              "direct approval command should be acknowledged"))
        (when (and thread (bt:thread-alive-p thread))
          (ignore-errors
            (bt:destroy-thread thread)))))))
...
(deftest codex-permission-reply-options-require-actionable-route
  (with-isolated-data
    (let* ((config (xmpp-cli/agent-config:set-notify-to
                    (xmpp-cli/agent-config:load-agent-config)
                    "friend@example.org"))
           (payload (xmpp-cli/json:parse-json
                     "{\"hook_event_name\":\"PermissionRequest\",\"model\":\"gpt-test\",\"turn_id\":\"turn-1\",\"session_id\":\"session-1\",\"cwd\":\"/tmp\",\"tool_name\":\"shell_command\",\"permission_mode\":\"default\",\"tool_input\":{\"command\":\"make test\"}}"))
           (context (list :tmux-socket "/tmp/tmux-1000/default"
                          :tmux-client-name "/dev/pts/45"
                          :tmux-client-tty "/dev/pts/45"
                          :tmux-session-id "$1"
           ...
           (notification
             (xmpp-cli/agent-codex:build-codex-notification
              ...
              :host "hbox"))
           (actionable
             (xmpp-cli/agent-codex:notification-with-permission-replies
              notification))
           (body (xmpp-cli/agent-codex:notification-body actionable))
           (route-code (getf (xmpp-cli/agent-codex:notification-route
                              notification)
                             :code)))
      (check (search "reply options:" body)
             "actionable permission notifications should include reply options")
      (check (search (format nil "allow: /choose ~a 1"
                             route-code)
                     body)
             "actionable permission notifications should include allow option")
      (check (search (format nil "deny: /choose ~a 2"
                             route-code)
                     body)
             "actionable permission notifications should include deny option")
      (check (search "/choose 1 (room)" body)
             "actionable permission notifications should include room syntax"))))
