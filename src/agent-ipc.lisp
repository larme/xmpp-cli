...
(defun load-daemon-lock ()
  (load-file-lock (daemon-lock-pathname) :label "agent/daemon.lock"))

(defun save-control (control)
  (ensure-private-directory (agent-directory))
  (write-yaml-atomically (control-pathname) (control-to-yaml control)))

(defun acquire-daemon-lock (token &key pid)
  (ensure-private-directory (agent-directory))
  (try-acquire-file-lock (daemon-lock-pathname) token :pid pid))

(defun daemon-lock-owned-p (token)
  (file-lock-owned-p (daemon-lock-pathname)
                     token
                     :label "agent/daemon.lock"))

(defun release-daemon-lock (token)
  (release-file-lock (daemon-lock-pathname)
                     token
                     :label "agent/daemon.lock"))

(defun delete-stale-daemon-lock (&optional expected-token)
  (delete-file-lock (daemon-lock-pathname)
                    expected-token
                    :label "agent/daemon.lock"))
